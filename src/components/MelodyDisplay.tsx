  ...
  return (
    <div className="glass-panel rounded-2xl p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className="font-semibold text-slate-900 text-lg">Generated Melody</h3>
        {isPlaying && (
      ...
      <div className="relative bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-4 mb-4 overflow-x-auto">
        <div className="relative h-32 min-w-full">
          {melody.notes.map((note, index) => {
            ...
            return (
              ...
              >
                {/* Note label on hover */}
                <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-slate-800 text-white text-xs px-2 py-1 rounded whitespace-nowrap z-20">
        ...
        <div className="absolute inset-0 pointer-events-none">
          {[0, 25, 50, 75, 100].map(position => (
            <div
              key={position}
              className="absolute w-px bg-slate-300/50"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        <div className="text-center">
          <div className="font-semibold text-slate-900">{melody.notes.length}</div>
          <div className="text-slate-600">Notes</div>
        </div>
        <div className="text-center">
          <div className="font-semibold text-slate-900">{melody.length}</div>
          <div className="text-slate-600">Beats</div>
        </div>
        <div className="text-center">
        ...
        <div className="text-center">
          <div className="font-semibold text-slate-900">{melody.key} {melody.scale}</div>
          <div className="text-slate-600">Key/Scale</div>
        </div>
      </div>
    </div>
  );
